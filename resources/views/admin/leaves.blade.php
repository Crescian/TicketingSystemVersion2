...
@section('hero-title')
    <h1><strong>WORK</strong> <em>LEAVE</em></h1>
@endsection
@section('hero-subtitle', 'Per-technician approved leave — skipped by scheduling alongside holidays and weekends.')

@section('hero-stats')
    <div class="d-flex gap-2 flex-wrap">
        <div class="stat-pill open">
            <span class="num">{{ $leaves->where('is_active', true)->count() }}</span>
            <span class="lbl">Active</span>
        </div>
        <div class="stat-pill all">
            <span class="num">{{ $leaves->count() }}</span>
...
@section('content')
    ...
    <div class="sla-main-layout">
        <div class="sla-left">
            <div class="sla-panel" id="levFormCard">
                <div class="sla-panel-head">
                    <div class="sph-icon" style="background:var(--rd)"><i class="bi bi-calendar-minus"></i></div>
                    <div>
                        <div class="sph-title" id="levFormTitle">Add Leave</div>
                        <div class="sph-sub">Skipped by scheduling for that technician only, same as holidays for everyone</div>
                    </div>
                </div>
                <form method="POST" id="levForm" action="{{ route('portal.leaves.store') }}">
                    @csrf
                    <input type="hidden" id="levMethod" name="_method" value="POST">

                    <div class="sf-field">
                        <label>Technician <span class="req">*</span></label>
                        <select class="form-select" name="technician_id" id="levTechnician" required>
                    ...
                    <div class="sf-field">
                        ...
                        <input type="date" class="form-control" name="date" id="levDate" required>
                    </div>

                    <div class="sf-field">
                        <label>Reason</label>
                        <input type="text" class="form-control" name="reason" id="levReason"
                               placeholder="e.g. Vacation, Sick leave">
                    </div>

                    <div class="d-flex gap-2 mt-2">
                        <button type="submit" class="btn-save"><i class="bi bi-check-lg me-1"></i><span id="levSaveText">Add Leave</span></button>
                        <button type="button" class="btn-cancel d-none" id="levCancelBtn" onclick="resetLevForm()">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="sla-right">
            <div class="sla-panel">
                <div class="sla-panel-head">
                    ...
                    <div>
                        <div class="sph-title">Leave</div>
                        <div class="sph-sub">Per-technician calendar — inactive entries are ignored by scheduling</div>
                    </div>
                </div>
                        ...
                        <table class="rules-table">
                            ...
                            <tbody>
                                @foreach($leaves as $leave)
                                    <tr class="{{ !$leave->is_active ? 'row-inactive' : '' }}">
                                        <td><div class="sub-name">{{ $leave->technician->name }}</div></td>
                                        <td>{{ $leave->date->format('M d, Y (D)') }}</td>
                                        <td>
                                            @if($leave->reason)
                                                {{ $leave->reason }}
                                            @else
                                        ...
                                        <td>
                                            <form method="POST" action="{{ route('portal.leaves.toggle', $leave) }}">
                                                @csrf @method('PATCH')
                                                <button type="submit" class="toggle-btn {{ $leave->is_active ? 'active' : 'inactive' }}">
                                        ...
                                        <td style="text-align:right">
                                            <div class="d-flex gap-1 justify-content-end">
                                                <button class="btn-row-action edit"
                                                        onclick="editLeave(
                                                            '{{ route('portal.leaves.update', $leave) }}',
                                                            '{{ $leave->technician_id }}',
                                                ...
                                                <form method="POST" action="{{ route('portal.leaves.destroy', $leave) }}"
                                                      onsubmit="return confirm('Delete this leave entry for &quot;{{ addslashes($leave->technician->name) }}&quot;?')">
                                                    @csrf @method('DELETE')
...
@section('scripts')
<script>
function editLeave(updateUrl, technicianId, date, reason) {
    document.getElementById('levForm').action = updateUrl;
    document.getElementById('levMethod').value = 'PUT';
    document.getElementById('levTechnician').value = technicianId;
    document.getElementById('levDate').value = date;
    document.getElementById('levReason').value = reason;

    document.getElementById('levFormTitle').textContent = 'Edit Leave';
    document.getElementById('levSaveText').textContent  = 'Update';
    document.getElementById('levCancelBtn').classList.remove('d-none');
    document.getElementById('levFormCard').classList.add('editing');
    document.getElementById('levFormCard').scrollIntoView({ behavior: 'smooth', block: 'start' });
}

function resetLevForm() {
    document.getElementById('levForm').action = '{{ route("portal.leaves.store") }}';
    document.getElementById('levMethod').value = 'POST';
    document.getElementById('levTechnician').value = '';
    document.getElementById('levDate').value = '';
    document.getElementById('levReason').value = '';

    document.getElementById('levFormTitle').textContent = 'Add Leave';
    document.getElementById('levSaveText').textContent  = 'Add Leave';
    document.getElementById('levCancelBtn').classList.add('d-none');
    document.getElementById('levFormCard').classList.remove('editing');
}
</script>
@endsection
